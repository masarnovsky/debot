...
fun returnListOfDebtorsForInlineQuery(chatId: Long, queryId: String) {
  ...
  val queries =
      debtors.keys.map { debtor ->
        createInlineQueryResultArticle(debtor, debtors[debtor]!!, currency)
      }
  bot.answerInlineQuery(queryId, queries)
}
...
fun deleteAllDebts(chatId: Long, messageId: Int) {
  ...
  editMessageTextAndInlineKeyboard(
      chatId, messageId, constructDeleteDebtorsMessageBasedOnDeletedCount(count))
}
...
fun showDebtorLogs(chatId: Long, name: String) {
  ...
  if (debtor != null) {
    ...
    val text = logs.reversed().fold(header) { temp, log -> temp + log.summarize() }.plus(footer)
    sendMessage(chatId, text)
  } else {
  ...
  return sourceLogs
      .map { sourceLog ->
        insertNewLogAndRecalculateDebt(
            destinationUser, sourceLog.getAmountAsRawValue(), sourceLog.comment)
      }
      .count()
}
...
fun processDeleteDebtorHistory(chatId: Long, messageId: Int, text: String) {
  ...
  editMessageTextAndInlineKeyboard(
      chatId, messageId, constructDeleteDebtorMessageBasedOnDeletedCount(name, count))
}
...
private fun findDebtorsWithLogs(chatId: Long): Map<Debtor, List<Log>> {
  ...
  transaction {
    ...
    debtors.forEach { map[it] = findLogsForDebtorByDebtorId(it.id!!) }
  }
...
private fun calculateAmountToAvoidNegativeBalance(
    totalAmount: BigDecimal,
    amount: BigDecimal
): BigDecimal {
  if (totalAmount > BigDecimal.ZERO &&
      amount < BigDecimal.ZERO &&
      (totalAmount + amount < BigDecimal.ZERO)) {
    return totalAmount.multiply(BigDecimal(-1))
  }
  return amount
}

private fun newLogRecord(
    name: String,
    amount: BigDecimal,
    comment: String,
    chatId: Long
...
fun insertNewLogAndRecalculateDebt(
    debtor: Debtor,
    amount: BigDecimal,
    comment: String
): Pair<Debtor, Log> {
